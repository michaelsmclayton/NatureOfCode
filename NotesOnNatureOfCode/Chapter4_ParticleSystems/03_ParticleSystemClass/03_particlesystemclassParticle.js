...
/*
  ...
  AT DEFINING A SINGLE PARTICLE CLASS
  ...
  is “dead” it will also have faded away.
    ...
    In addition to the lifespan variable, we’ll also need one additional function: one that can be
  queried (returning a true or false answer) as to whether the particle is alive or dead. This will
  become  handy when we write the ParticleSystem class (whose task is to manage the list of particles
  themselves. In this code, the this isDead() function is quite simple; we just need to check and see
  if the value of lifespan is less than 0. If it is we return true, if not we return false.
*/

class Particle {

  constructor(emitterOrigin, startingColour, acceleration) {
    this.acceleration = acceleration; // Initialisee with a downward acceleration (constant to simulate gravity)
    this.velocity = createVector(random(-2, 2), random(-1, 1)); // Initalise with random vecor for velocity
    this.position = emitterOrigin.copy(); // Set the starting position at the emitter origin
    this.lifespan = 255.0; // Intialise with a full lifespan
  ...
  // Method to update position and lifespan
  update() {
  ...
  isDead() {
    if (this.lifespan < 0.0) { // If lifespan is below 0...
      return true; // ..kill
    } else {
      return false;
    }
