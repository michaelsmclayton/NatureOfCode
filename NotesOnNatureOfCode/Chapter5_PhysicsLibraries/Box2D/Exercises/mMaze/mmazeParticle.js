...
class Particle {
  ...
  applyBounce() {
    //this.applyForce(new box2d.b2Vec2(-2000, -10000))
  }
  ...
  display() {
    ...
    let alpha,
        distanceFromXCenter = Math.abs(pos.x-width/2),
        distanceFromYCenter = Math.abs(pos.y-height/2),
        xAlpha = 255-(distanceFromXCenter),
        yAlpha = 255-(distanceFromYCenter*.75);
    if (xAlpha<yAlpha){
      alpha = xAlpha;
    } else {
      alpha = yAlpha
